...
- **条件付き代入**（`at_hash` / `nonce` / `auth_time` / `acr` / `amr` / `azp`）:
  OP 側がその値を持たないとき（`undefined`）に**上書きが起きない**。
  `azp` は単一 audience の既定構成で常に `undefined` になるため、この条件に該当する
  （OIDC Core 1.0 §12.2 は、元の ID Token に無かった `azp` を refresh 再発行で
  出してはならない（MUST NOT）とも定めており、注入されると §2 と §12.2 の両方に反する）

今日はユーザークレームが `SCOPE_CLAIMS_MAP` の allowlist を通るため、
条件付きの 6 クレームには到達できない（実害は無い）。
...
`at_hash` を含む条件付き代入の 6 クレームについては、
