...
- [x] introspection ルートの認証パイプライン直後に「登録方式が `'none'` のクライアントを `invalid_client`（401 + WWW-Authenticate）で拒否する」ステップを追加する
- [x] JSON 経路（RFC 7662）と JWT 経路（RFC 9701）の両方より前に効く位置に置く
- [x] revocation ルートには適用しない（RFC 7009 §2.1 で public client は正当）
- [x] ルートコメントの「Confidential client only」を実挙動として担保する
- [x] specification.md と introspection-caller-authorization-and-disclosure.md の「クライアント認証必須で MUST を満たす」前提を訂正する
- [x] jwt-introspection-response の実装解説（ja / en）の掲載コードと説明を同じ変更内で更新する

## テスト要件

- [x] `should reject an introspection request that presents only a public client_id`（401 / invalid_client。4 フレームワーク）
- [x] `should reject a public client introspection request even when it asks for the JWT response`（`--enable jwt-introspection-response` 時）
- [x] `should keep answering an authenticated confidential client as before`（回帰: 既存の confidential クライアントのケースが全て緑のまま）
- [x] 生成 conformance テストへ public client_id のみのケースを追加する
...
- core に `requireConfidentialIntrospectionCaller` を追加（`packages/core/src/introspection.ts`）
