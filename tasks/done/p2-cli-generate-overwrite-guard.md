...
- [x] `parseArgs` に `--force` と `--dry-run` を追加する
- [x] `writeGeneratedFiles` の前に、出力予定パスのうち既に存在するものを列挙する
- [x] 既存ファイルが 1 件以上あり `--force` が無い場合は、**1 バイトも書き込まずに**中断し、
...
- [x] `--force` 指定時はログを出し分ける（`Created:` / `Overwritten:`）
- [x] `--dry-run` は書き込みを行わず、出力予定のファイル一覧と、それぞれが新規か上書きかを表示する
- [x] `setup` コマンドでも同じガードが効くようにする（配線より前に中断する）

### B. 生成元バージョンの記録（マニフェスト方式）

ヘッダコメント方式は全テンプレートの期待出力テストに波及するため、マニフェストファイル方式を採る。

- [x] 生成物に `.maronn-openid-connect.json` を追加し、次を記録する
...
- [x] `cliVersion` は `packages/cli/package.json` の `version` を参照する
      （バンドル方法に応じて、ビルド時定数化するか `createRequire` で読むかを実装時に決める）
- [x] 生成日時は含めない（同じ入力から同じ出力になる決定性を壊さないため）
- [x] マニフェストは上書きガードの対象外とし、`--force` 無しでも常に更新する
      （利用者が編集する種類のファイルではないため）
- [x] `generator.test.ts` などで生成ファイル数を固定しているテストを更新する

### C. ドキュメント

- [x] `docs/library-document/src/content/docs/guides/cli.md` に次を追記する
...
- [x] `should exit with a non-zero code when the output directory already contains generated files`
- [x] `should leave existing files untouched when generate is refused`
      （既存ファイルの内容が変更前と完全一致することを `toBe` で固定）
- [x] `should list every existing file in the refusal message`
      （表示されるパス集合を `toEqual` で固定）
- [x] `should overwrite existing files when --force is given`
- [x] `should log Overwritten for an existing file when --force is given`
- [x] `should log Created for a new file when --force is given`
- [x] `should not write any file when --dry-run is given`
- [x] `should list the files it would write when --dry-run is given`
- [x] `should refuse setup as well when the output directory already contains generated files`
- [x] `should write a .maronn-openid-connect.json manifest recording the cli version, framework and features`
      （マニフェストの内容を `toEqual` で固定。`features` は全キーを具体値で指定する）
- [x] `should update the manifest even when generate is run with --force`
- [x] 既存の生成テスト（ファイル数・ファイル一覧を固定しているもの）をマニフェスト追加後の値へ更新する

## 完了条件

- [x] 既存の出力ディレクトリに対する `generate` が、`--force` 無しでは 1 バイトも書き込まずに非ゼロ終了すること
- [x] `--dry-run` が書き込みを行わないこと
- [x] 生成物に `.maronn-openid-connect.json` が含まれ、`cliVersion` / `framework` / `features` が記録されていること
- [x] 下記がすべてパスすること
...
- [x] 4 サンプル（`hono-cloudflare` / `express-flyio` / `fastify-flyio` / `nextjs-vercel`）の
      再生成手順が、マニフェスト追加後も破綻しないことを確認すること
      （サンプル配下の生成物を再生成する運用がある場合は `--force` 前提に更新する）
- [x] `docs/library-document/src/content/docs/guides/cli.md` に上書き挙動が明記されていること

## 実装メモ（2026-09-17 実装済み）

- マニフェストの書き出しは `generator.ts` ではなく CLI 層（`index.ts` の `run()`）に置いた。`generate()` のプログラム利用者は framework / features を自分で持っているため記録の必要が無く、`generator.test.ts` と各フレームワークのファイル一覧固定テストを変更せずに済む
- マニフェストには `scopes`（`--scope` 宣言。PR #74 で追加されたオプション）も記録する
- 実装解説: notes リポジトリ `implementation-guides/cli-overwrite-guard-and-manifest.{ja,en}.md`
