...
2026-08-24 に依頼者の判断が変わり、ルーティーン2件の更新をスコープへ戻して実施した。
同日、分離コミット `2cbf324` を origin/main へ push し、公開ツリーの直下が「残すもの」だけになったことを GitHub Contents API で確認した。
残るのは、クラウド環境のセットアップスクリプト（タスク4-1）と、クラウドセッションでの実地確認（タスク4-3、タスク5-1）の 2 つ。
どちらも Claude Code の cloud environment を Web UI から操作する必要があり、`RemoteTrigger` の API では扱えない。依頼者の手作業として残す。
...
7. Claude Code on the Web の対応とルーティーンの更新は行わない（決定8で一部撤回）
8. 2026-08-24、ルーティーン2件のプロンプト更新はスコープへ戻す。クラウド環境の設定と実地確認は依頼者が手作業で行う
...
| 3 | 昇格レビューツールの削除 | 2 |
| 4 | クラウド実行からの notes 取得（手順書のみ実施。環境設定と実地確認は依頼者の手作業） | 2 |
| 5 | ルーティーン 2 件の修正 | 2〜4 |
...
タスク4-2 の手順書は notes リポジトリの `docs/cloud-session-bootstrap.md` に置いた。
タスク4-1 と 4-3 は cloud environment の設定とクラウドセッションの起動が要る。
`RemoteTrigger` はルーティーンしか扱えず、環境のセットアップスクリプトは Web UI からしか変更できないため、依頼者の手作業として残す。
...
2026-08-24 に 2 件とも `RemoteTrigger` の update で差し替えた。
どちらも `cron_expression` が空で `next_run_at` も未設定のため、現状は手動起動でしか動かない。
...
- [x] 上記を反映したプロンプトへ `RemoteTrigger` の update で差し替える（2026-08-24）
