...
|---|---|
| 追加 | （なし） |
| 変更 | _oidc-provider/config.ts, _oidc-provider/conformance.test.ts, _oidc-provider/routes/discovery.ts, _oidc-provider/routes/token.ts |
| 削除 | （なし） |
...
+import { tokenExchangeConfig } from './routes/token';
...
diff --git a/default-op/_oidc-provider/routes/discovery.ts b/with-token-exchange/_oidc-provider/routes/discovery.ts
index 6561812..9869c58 100644
--- a/default-op/_oidc-provider/routes/discovery.ts
+++ b/with-token-exchange/_oidc-provider/routes/discovery.ts
