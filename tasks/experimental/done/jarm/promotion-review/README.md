...
> **このディレクトリは `pnpm review:experimental jarm` が生成した機械生成物です。**
> `decision.md` 以外はツールが再生成のたびに作り直します。手で編集しないでください。

experimental 機能 `jarm` を experimental から外してよいか（昇格させてよいか）を、
...
- [tasks/experimental/done/jarm/specification.md](../../../../../tasks/experimental/done/jarm/specification.md)
- [tasks/experimental/done/jarm/understanding-guide.md](../../../../../tasks/experimental/done/jarm/understanding-guide.md)
- [tasks/experimental/done/jarm/sources.md](../../../../../tasks/experimental/done/jarm/sources.md)
- [tasks/experimental/done/jarm/review-log.md](../../../../../tasks/experimental/done/jarm/review-log.md)
- [tasks/experimental/done/jarm/state.yaml](../../../../../tasks/experimental/done/jarm/state.yaml)

## 1. この機能を構成するコードの地図

### 1.1 experimental パッケージ本体（ロジック層）

実装（[packages/experimental/src/jarm](../../../../../packages/experimental/src/jarm)）:
...
`packages/cli` 内で `jarm` に言及しているファイル（言及回数は機械カウント）:
...
`--enable jarm`（= `with-jarm`）を足したときに生成コードへ入る差分だけを、
...
- hono: app.ts, apply.ts, config.ts, resolvers.ts, routes/introspection.ts, routes/jwks.ts, routes/login.ts, routes/revocation.ts, routes/token.ts, routes/userinfo.ts, store.ts, views.ts
- express / fastify: app.ts, apply.ts, config.ts, node-adapter.ts, resolvers.ts, routes/introspection.ts, routes/jwks.ts, routes/login.ts, routes/revocation.ts, routes/token.ts, routes/userinfo.ts, store.ts, views.ts, web-router.ts
...
| ファイル | 言及回数 |
|---|---|
| [tests/e2e/apps/client.mjs](../../../../../tests/e2e/apps/client.mjs) | 43 |
| [tests/e2e/specs/jarm.spec.ts](../../../../../tests/e2e/specs/jarm.spec.ts) | 30 |
...
`pnpm review:experimental jarm --check` が失敗する場合、パケット生成後に実装が変わっています。
