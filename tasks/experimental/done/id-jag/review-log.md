...
- **日付**: 2026-08-30（actor_token 種別の一律受理への方針転換と同日）
- **観点**: Review 3 で導入した「id_token とそれ以外」の二分（`allowCustomActorTokenTypes` と組込み経路の非バイパス）が、仕様と利用者の意図に照らして妥当か
- **確認資料**:
  - RFC 8693 §2.1 / §3 — actor_token_type は「トークンの種別を示す識別子」であり、§3 は access_token / refresh_token / id_token / saml1 / saml2 を、RFC 7519 §9 は jwt を定義する。仕様は種別ごとの処理規則を定めておらず、id_token を特別扱いする根拠は無いことを確認
  - draft-ietf-oauth-identity-assertion-authz-grant-04 §9.7 — actor 拡張への要請（権威の過大表明の防止、act の導出、開示最小化）はいずれも種別に依存しない
  - 本リポジトリの設計方針（生成コードは読んで差し替えられること） — 種別ごとの分岐は生成コード側に置けば、デプロイが自分の判断で受理範囲を決められる
- **指摘**:
  1. `allowCustomActorTokenTypes` は「仕様の一部の種別だけを一級市民として扱う」概念であり、RFC 8693 に対応する規定が無い。ライブラリが仕様に無い区別を持ち込んでいる
  2. 組込み id_token 検証の非バイパスは、デプロイが自分のポリシーで actor 検証を決められないという副作用を持つ。差し替え不能な検証は本リポジトリの生成コード方針とも合わない
  3. 「id_token 用の組込み経路」と「それ以外用のリゾルバ経路」の二重構造は、テストとドキュメントの両方で説明量を倍にしていた
- **修正**（Review 3 の該当箇所を破棄して差し替え）:
  1. `allowCustomActorTokenTypes` を廃止した。`actor_token_type` は `ACTOR_TOKEN_TYPES_SUPPORTED`（RFC 8693 §3 / RFC 7519 §9 の 6 種）を種別で区別せず受理し、一覧外の識別子だけを `invalid_request` で拒否する
  2. 組込み経路を廃止し、受理したすべての種別が単一の `actorTokenResolver` を通る形にした。ID トークンの検証実装（`resolveIdJagActor`）は「リゾルバ実装のひとつ」として残し、生成コードが既定リゾルバとして配線する（差し替え・拡張が可能）
  3. リゾルバ未設定の構成は、内容を検証できない以上どの actor_token も有効になり得ないため、固定文言の `invalid_request` とした（fail-safe）
- **残リスク**:
  - 既定リゾルバを差し替えると ID トークンの検証水準もデプロイ側の責任になる。ライブラリ側に強制する手段は無く、生成コードの既定実装とドキュメントの実装指針で補う
  - Review 3 の「非バイパス」に関する記述は本レビューで無効。仕様書・理解資料・実装解説は本レビューの内容に更新済み
- **判定**: Pass with changes（方針転換を実装・文書へ同日反映）
- **次回可能日**: 実装後（実装との整合レビュー）
