...
- **強制ログアウト（DoS）**: リンクを踏ませるだけの攻撃は確認画面で止まる。即時ログアウトには被害者本人の ID Token が必要で、それを持つ攻撃者はヒント検証を通せるが、`sub` がセッションと一致するのは被害者本人のブラウザだけなので、他人のブラウザのセッションは消せない。被害者の ID Token を盗んだ攻撃者が被害者本人のブラウザにリンクを踏ませれば即時ログアウトは成立するが、これは有効なヒントをログアウト権限の証拠とする仕様の信頼モデルの帰結として受容する（ID Token が盗まれた時点で、ログアウト強要より重大な被害が既に成立している）
- **オープンリダイレクト**: リダイレクトは検証済みクライアントの登録値と完全一致した場合だけ。ヒントが無効なら一致しても飛ばさない。正規化やプレフィックス一致を入れないのは、authorize の `redirect_uri` 検証と同じ判断
- **確認画面への CSRF**: 承認 POST を偽造されると確認画面の意味がなくなるため、確認画面の描画時に発行する cookie と hidden `csrf_token` の対で守る（Device の binding cookie と同じモデル）。攻撃者は自分のブラウザで有効な対を取れるが、その cookie を被害者のブラウザへ植え付けられないため、クロスサイトの偽造 POST は照合で落ちる
