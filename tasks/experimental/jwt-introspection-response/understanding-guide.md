...
- **有効なトークンでも JWT 経路では `active: false` になり得る**。生成 OP のアクセストークン `aud` は既定で UserInfo エンドポイント URL と要求されたリソース値だけを含み、client_id は入らない。このため発行先本人以外の RS が JWT で問い合わせると、トークンが有効でも audience 制限で `{ active: false }` になる。その RS へ開示したい場合は、認可リクエストの `audience` パラメータなどで、その RS の client_id を `aud` に入れてトークンを発行する
