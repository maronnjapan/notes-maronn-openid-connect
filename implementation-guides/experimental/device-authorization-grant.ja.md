# Device Authorization Grant 実装解説

この文書は、`packages/experimental` に実装した **OAuth 2.0 Device Authorization Grant**（RFC 8628）について、何を実装したのか、どう実装したのかを、関連するコードの全文とともに説明する。

この文書が全文を載せるのは次のコードである。

- `packages/experimental/src/device-authorization-grant/` の実装 6 ファイル、テスト 4 ファイル、テスト用フィクスチャ 1 ファイルのすべて
- CLI の `--enable device-authorization-grant` が生成コードへ注入する差分の全文（hono）
- Device Authorization Grant の E2E テストスペックの全文

core 本体、E2E 共有ハーネス、他フレームワークの生成差分は全機能で共有される基盤なので、リンクで参照する。

## 機能の概要

Authorization Code Flow は「ユーザーの操作するブラウザをリダイレクトで OP に連れて行き、また連れて帰る」ことを前提にしている。
ところがスマート TV、CLI ツール、IoT 機器のように、ブラウザを持たない、あるいは文字入力が困難なデバイスでは、この前提が成立しない。

Device Authorization Grant はこの制約を、認可の場をユーザーの別デバイス（スマートフォンや PC のブラウザ）へ移すことで解決する。
フローは 3 本の並行する線でできている。

1. デバイスは OP のデバイス認可エンドポイントへ POST し、**device_code**（デバイスがトークン取得に使う長い secret）と **user_code**（ユーザーが書き写す短いコード、例 `WDJB-MJHT`）のペアを受け取り、user_code と検証用 URL を画面に表示する
2. ユーザーは自分のスマートフォン等で検証用 URL を開き、user_code を入力し、ログインして承認（または拒否）する
3. その間デバイスは、トークンエンドポイントを `grant_type=urn:ietf:params:oauth:grant-type:device_code` でポーリングし続け、承認が済んだ回のポーリングでトークン一式を受け取る
...
デバイス                        OP                        ユーザーのブラウザ
    ...
    │ <───────────────────────── │                              │
    │ 画面に user_code を表示     │        GET /device           │
    │                            │ <──────────────────────────  │
    │ POST /token（ポーリング）    │        user_code 入力         │
    │ ─────────────────────────> │        ログイン・承認          │
...
- スマート TV やセットトップボックスのアプリが、ユーザーのスマートフォンで認可を受けるフローの検証
- CLI ツールのログイン（`gh auth login` のような体験）を自前の OP で再現する検証
- キーボードを持たない IoT 機器のプロビジョニングでの認可設計の検証

### 実装スコープと非目標

実装したのは次の範囲である。

- デバイス認可エンドポイントの処理（RFC 8628 §3.1 / §3.2）
- 検証 UI のステップ関数（§3.3。user_code の照合、ブラウザバインディング、CSRF、承認と拒否）
- トークンエンドポイントのデバイスコード分岐（§3.4 / §3.5 の状態機械）
- user_code の生成と正規化（§6.1）

既知の制限として、scope の省略には対応しない。
RFC 8628 §3.1 では scope は OPTIONAL だが、本 OP は認可エンドポイントと同じプロファイル制限（scope 必須、`openid` 必須）をデバイス認可にも課している。
また、検証 UI は scope の部分承認を提供しない（承認は要求 scope 全体に対して行う）。
...
| ファイル | 役割 |
|---|---|
| `store.ts` | ストア契約、grant type URN、user_code の文字種定数 |
| `errors.ts` | エラー型 2 種（OAuth エラーと検証 UI エラー） |
| `user-code.ts` | user_code の生成、整形、正規化 |
| `device-authorization-request.ts` | デバイス認可エンドポイントの処理 |
| `verification.ts` | 検証 UI のステップ関数 |
| `device-code-grant.ts` | トークンエンドポイントの状態機械 |
| `index.ts` | 公開 API の再エクスポート |
| `test-helpers.ts` | テスト用フィクスチャ（配布物には含めない） |

この機能に固有の設計判断は、検証 UI の CSRF 防御の主役を**ブラウザバインディング**に置いたことである。
device フローの user_code は「フローを開始した主体」が設計上必ず知っている識別子で、その主体こそが攻撃者になり得る。
レコードに紐づけただけの CSRF トークンでは、攻撃者自身が `POST /device` を叩いて正規のトークンを取得できてしまい、防御にならない。
そこで user_code の照合成功時に **bindingSecret** を発行し、生値はブラウザだけが持つ HttpOnly Cookie に載せ、レコードには SHA-256 ハッシュだけを保存する。
以後のログインと承認は、Cookie の生値がレコードのハッシュと一致しない限り実行されない。
クロスサイトの偽造 POST は被害者ブラウザの Cookie を運べない（SameSite=Lax）うえ、そもそも被害者ブラウザは当該レコードの Cookie を持っていないため、トークンの秘匿に依存せず遮断できる。
authorize フローの transaction-binding が opt-in のハードニングであるのに対し、こちらが常時有効なのは、user_code が開始者に既知であることを前提とする device フローではバインディングがベースライン要件になるからである。

情報漏えいの遮断も一貫している。
user_code の照合失敗は、未知でも期限切れでも使用済みでも同じ文言で返し（RFC 8628 §5.1）、device_code の解決失敗は、不存在でも他クライアントの発行分でも同じ文言の `invalid_grant` にする。
生成した user_code はログにも例外メッセージにも残さない。

## 実装コードの全文と解説

### store.ts（ストア契約と定数）

レコードは状態機械（`pending` / `approved` / `denied`）を持ち、承認時にだけ `subject` / `authTime` / `approvedScope` / `grantId` が入る。
user_code は正規化済みの照合キー（`userCode`）と表示形式（`userCodeDisplay`）を分けて保存する。

文字種 `BCDFGHJKLMNPQRSTVWXZ` は RFC 8628 §6.1 が推奨する base-20 で、視覚的に紛らわしい文字と母音を除いてある。
母音が無いので、偶発的に意味のある単語が生成されることもない。
8 文字で 20^8 ≈ 2.6 × 10^10 のエントロピーを確保する。

ストア契約では、トークン発行時の単回使用を `consume`（取得と削除が atomic）で強制する。
`update` の read-modify-write が atomic でない実装では並行ポーリング時に間隔の強制が甘くなり得るが、状態遷移と `consume` が守られていればセキュリティ特性は保たれる、という許容範囲もコメントで明示している。
...
エラー型は応答形式の違いで 2 つに分かれる。
`DeviceAuthorizationError` はバックチャネル（デバイス認可エンドポイントとトークンエンドポイント）用の OAuth エラーで、RFC 8628 §3.5 が登録した 4 値（`authorization_pending` / `slow_down` / `access_denied` / `expired_token`）と RFC 6749 §5.2 の既存値を持ち、常に 400 で返す。
`DeviceVerificationError` は検証 UI（HTML ページ）用で、HTTP ステータス（401 / 403）をそのまま持つ。
...
### user-code.ts（user_code の生成と正規化）

`generateUserCode` は CSPRNG で 1 文字ずつ選ぶ。
20 は 256 の約数ではないため、単純な剰余では文字ごとの出現確率が偏る（modulo bias）。
偏りは 20^8 のエントロピー主張を弱めるので、剰余の周期に収まらないバイトを破棄して引き直す rejection sampling で選んでいる。
文字種と長さを設定値にしていないのは、エントロピー保証を利用者の設定ミスで壊さないためである。

`normalizeUserCode` は、ユーザー入力の大文字小文字、空白（全角を含む）、ハイフン類の揺れを吸収して照合キーへ揃える。
文字種の妥当性検証をあえて行わないのは、実在しないコードと不正な形式のコードを応答から区別させないためである。

`generateUniqueUserCode` は既存の pending レコードと衝突しないコードを引き直しつきで生成する。
衝突したまま保存すると `findByUserCode` が別レコードを返し、あるユーザーの承認が別デバイスへ渡り得る。
上限回数まで引いても衝突が続いた場合は握りつぶさず throw する（生成コードは 500 にする）。
...
### device-authorization-request.ts（デバイス認可エンドポイント）

ステップ関数は 5 つで、合成関数 `processDeviceAuthorizationRequest` が仕様順に呼ぶ。

1. `validateDeviceGrantAllowed`：登録済み `grantTypes` にデバイスグラントの URN が無ければ `unauthorized_client`
2. `validateDeviceAuthorizationScope`：scope 必須と `openid` 必須を検証し、空白区切りの分解と重複除去を core の認可エンドポイントと同じ規則で行う
3. `applyOfflineAccessPolicy`：許可条件（refresh-token feature が有効、かつクライアントが `refresh_token` grant を登録済み）を満たさない `offline_access` を、エラーにせず scope から除去する（OIDC Core §11 の「無視する」の実装）。デバイスフローでは検証 UI の承認画面が明示同意そのものなので、`prompt=consent` 相当の事前条件は課さない
4. `createDeviceAuthorizationRecord`：256 ビットの device_code と一意な user_code を生成してレコードを保存する
5. `buildDeviceAuthorizationResponse`：§3.2 の応答（`verification_uri` は `<issuer>/device`、`verification_uri_complete` は user_code をクエリに載せた形）を組み立てる

`verification_uri_complete` は OPTIONAL だが常に返す。
user_code がクエリに載るためブラウザ履歴やプロキシのログに残り得るが、user_code はワンタイムかつ短命で、承認操作をした本人以外には価値を持たない、という判断をコメントに残している。

クライアント認証がこのモジュールに無いことにも触れておく。
認証は生成コード側の共有パイプライン（core の `extractClientCredentials` からの 4 関数）で済ませ、認証済みクライアントをここへ渡す分担にしている。
...
検証 UI の 3 ルート（`GET/POST /device`、`POST /device/login`、`POST /device/approve`）が呼ぶ関数群である。

`findPendingRecordByUserCode` は user_code を正規化して照合し、承認可能な pending レコードだけを返す。
未知、期限切れ、非 pending はすべて `null` で、呼び出し側は理由を区別せず同一文言でフォームを再表示する。

`issueVerificationBinding` は照合成功時に bindingSecret と CSRF トークンをペアで発行し、レコードの値を置き換える（回転）。
別ブラウザが同じ user_code で照合し直すと先のブラウザのバインディングは無効になる（last-writer-wins）が、user_code を知る者がレコードの承認や拒否を左右できるのは RFC 8628 のモデルそのものなので、機能ではなく制約として受け入れている。

`validateVerificationBinding` は Cookie の生値をハッシュ化してレコードと比較する。
比較のタイミング差から保存値を積み上げる攻撃は、ハッシュ比較なので原像計算が必要になり成立しない。
`validateVerificationCsrfToken` は hidden フィールドの照合で、主防御のバインディングに重ねる多層防御である。

`recordDeviceLoginFailure` はデバイス用ログインの失敗をレコード単位で計数し、上限を超えたら `denied` へ遷移させてフローを終わらせる（デバイスは次のポーリングで `access_denied` を受け取る）。
デバイスグラントを許可されたクライアントならレコードを無制限に発行できるため、集計上のパスワード試行回数は無制限に残る。
この残存面は既存の `/login` ルートと同一で、subject 単位のスロットリングは別タスクの責務としている。

`approveDeviceAuthorization` / `denyDeviceAuthorization` は一方向の状態遷移である。
承認時は `subject` / `authTime` / `approvedScope` / `grantId` を確定し、用済みになったバインディングと CSRF トークンをレコードから落とす。
`grantId` を新規発行するのは、既存の revocation 機構が grant 単位失効をそのまま適用できるようにするためである。
...
トークンエンドポイントの grant 分岐に載る処理で、`evaluateDeviceCodeState` が RFC 8628 §3.5 の状態機械を上から順に評価する。

1. 期限切れは `expired_token`（レコード削除）。ポーリング過速より先に評価するのは、期限切れレコードの interval を増やしても意味がなく、デバイスへはフロー終了を伝えるべきだからである
2. ポーリング過速は `slow_down`。§3.5 の「interval を 5 秒増やす MUST」をサーバー側でも強制するため、レコードの interval に 5 を足して保存する
3. pending は `authorization_pending`（`lastPolledAt` を更新）
4. denied は `access_denied`（レコード削除）
5. approved は結果を返す。単回使用を強制するため atomic な `consume` で回収し、並行リデンプションでは先勝ちの 1 本だけがトークンを得る（後続は `invalid_grant`）

`resolveDeviceCodeRecord` は device_code の発行先クライアント紐付け（§3.4）を検証し、不存在と他クライアント発行分を同一文言の `invalid_grant` にする。
...
subpath export `@maronn-openid-connect/experimental/device-authorization-grant` の実体である。
...
### test-helpers.ts（テスト用フィクスチャ）

4 つのテストファイルが同じストア実装とレコード工場を必要とするため、機能内で共有している。
tsconfig の exclude により配布物（`dist`）には含まれない。
...
`user-code.test.ts` は生成と正規化を固定する。
文字種と長さ、表示形式、正規化の揺れ吸収（小文字、空白、全角スペース、ハイフン）、衝突時の引き直しと上限到達時の throw、生成値が例外メッセージへ漏れないことを検証する。
...
`device-authorization-request.test.ts` はデバイス認可エンドポイントを固定する。
grant 許可の検証、scope の必須と正規化、offline_access ポリシーの 4 象限（feature 有効性 × クライアント登録）、レコード生成の内容と期限、応答の形（`verification_uri_complete` の組み立てを含む）、合成関数の一気通貫を検証する。
...
`verification.test.ts` は検証 UI のステップ関数を固定する。
照合の全失敗種別が `null` になること、バインディングの発行と回転と検証（ハッシュ保存、生値不在時の 403）、CSRF 照合、ログイン失敗の計数と上限での `denied` 遷移、承認と拒否の状態遷移（確定するフィールド、用済み値の消去、非 pending の拒否）を検証する。
...
`device-code-grant.test.ts` はトークンエンドポイントの状態機械を固定する。
§3.5 の 5 状態それぞれの応答とレコードへの副作用、判定順序（期限切れが過速より先）、slow_down での interval 増加、承認済みレコードの consume と並行リデンプションの先勝ち、クライアント紐付けの同一文言を検証する。
...
`maronn-oidc generate <framework> --enable device-authorization-grant` を指定すると、生成コードに次が入る。

- **routes/device-authorization.ts の追加**：`POST /device_authorization` のハンドラ。共有パイプラインでのクライアント認証、ステップ関数の呼び出し、`deviceAuthorizationConfig`（`deviceCodeExpiresIn: 600` / `pollInterval: 5` / `maxLoginAttempts: 5`）を持つ。user_code 推測面のレート制限を意図的にデプロイ層へ委ねる判断（インスタンス間で共有メモリの無いランタイムでは、プロセス内カウンタが誤った安心感を与えるだけになる）もこのファイルのコメントに明記される
- **routes/device.ts の追加**：検証 UI の 3 ルート。user_code 入力フォーム、デバイス用ログイン、承認画面と、バインディング Cookie の発行と検証を実装する
- **routes/token.ts の変更**：core の `validateGrantTypeSupported` より前に `grant_type=urn:ietf:params:oauth:grant-type:device_code` の分岐が入り、`processDeviceCodeGrant` の結果を core の発行パイプラインへ流す
- **routes/discovery.ts の変更**：`device_authorization_endpoint` と grant type の広告が加わる
- **store.ts の変更**：`deviceAuthorizationStore` が加わる
- **app.ts / apply.ts / views.ts の変更**：ルートのマウント、メソッド制約、検証 UI のビュー
- **conformance.test.ts の変更**：上記の挙動一式を契約テストとして固定する

### 生成コードに入る差分の全文（hono）

以下は、デフォルト構成（`default-op`）と `--enable device-authorization-grant`（`with-device-authorization-grant`）の生成結果の差分そのもので、`--enable device-authorization-grant` が生成コードに足すもののすべてである。
...
この機能はすべてのサンプル（express-flyio / fastify-flyio / hono-cloudflare / nextjs-vercel）で有効になっている。

## E2E テストの全文

E2E テストは、デバイス役を HTTP リクエストで、ユーザー役を実ブラウザで演じ、次を固定する。

- 別デバイスのブラウザで承認した後、ポーリング中のデバイスがトークン一式を受け取ること
- ユーザーが拒否したとき、デバイスへ `access_denied` が返ること
- ユーザーが未決の間、`authorization_pending` が返り続けること
- バインディング Cookie を持たないブラウザでは検証ステップが実行できないこと
- 別クライアントが提示した device_code が拒否されること
...
- 利用者向けドキュメント：[docs/library-document experimental/device-authorization-grant.md](../../library-document/src/content/docs/experimental/device-authorization-grant.md)
- 仕様検討文書：[tasks/experimental/done/device-authorization-grant/](../../../tasks/experimental/done/device-authorization-grant/)
- 昇格レビューパケット：[tasks/experimental/done/device-authorization-grant/promotion-review/](../../../tasks/experimental/done/device-authorization-grant/promotion-review/README.md)
- パッケージ全体の設計規約：[package-overview.ja.md](./package-overview.ja.md)
- English version: [device-authorization-grant.en.md](./device-authorization-grant.en.md)
